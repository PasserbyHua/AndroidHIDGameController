...
- **完整手柄键位**：
  - 16 个按键（A、B、X、Y、LB、RB、LT、RT、BACK、START 等）
  - D-Pad 十字方向键（Hat Switch）
  - 双轴摇杆（X / Y，范围 -127 ~ +127）
- **多种操控界面**：
  - **测试界面**：查看连接状态、重力传感器数据，连接/断开已配对设备
  - **滑动按键界面**：通过手指滑动区域触发 Hat 左/右、B、MENU、X、A 按键
  - **重力摇杆界面**：利用手机重力传感器控制摇杆 XY 轴（带死区与角度映射），支持触摸区域按键
  - **滑块摇杆界面**：屏幕滑动控制左摇杆 X 轴（带死区），支持触摸区域按键
- **125Hz 高频轮询上报**：确保手柄操作响应流畅
...
4. 各操作界面将触摸/传感器输入写入 `BluetoothHidGamepad` 的状态寄存器，由 125Hz 轮询循环调用 `sendReport()` 持续上报
...
3. 连接成功后状态显示为 **“已连接”**，此时目标设备应能识别到名为 `GameController` 的手柄
